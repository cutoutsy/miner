...
public class EncodeUtil {
    ...
    public ContentPojo getWebPageCharset(byte[] byteArray,String content_type_string) {
        //content_type_string会在下载器中传过来
        ContentPojo contentPojo = new ContentPojo(byteArray,content_type_string);
        // byte[] byteArray = contentPojo.getByteArray();
        if (byteArray == null || byteArray.length == 0|| byteArray.length <= StaticValue.url_data_min_byte_length) {
            // 最后的判断是去掉URL对应的数据内容太短的情况
            return contentPojo;
        }
        ...
        try {
             ...
             */
            charsetByMetadata = getCharsetByMetadata(contentPojo.getContent_type_string());
            if (charsetByMetadata != null) {
                // 说明从metadata是得到charset，直接用，不进行后边的计算了
                ...
                isFoundCharset = setCharset(contentPojo, lastEncoding,byteArray);
                ...
                if (!isFoundCharset) {
                    ...
                    if (oldCharset == StaticValue.SYSTEM_ENCODING) {// 如果得到oldCharset是UTF-8，这种情况即实际和charset不符，则默认再按GBK处理一次，默认只有国内有这种情况
                        ...
                        title = getUrlTitle(StaticValue.GBK_ENCODING,
                                byteArray, bodyFront);
                        if (regexUtil.isContainFreqChineseChar(bodyFront.getBodyFront())) {
    ...
    public boolean setCharset(ContentPojo contentPojo, String lastEncoding,byte[] bytes) throws Exception {
...
//	}
    ...
    // 设置contengPojo的title
    public String getUrlTitle(String lastEncoding, byte[] bytes,StringTemp4BodyFront bodyFront) throws Exception {
