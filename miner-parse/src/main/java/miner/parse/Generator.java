...
public class Generator {
    ...
    public void both_idea(String document){
        Iterator<RuleItem> it = parse_rule_set.iterator();
        while (it.hasNext()) {
            RuleItem tmp_rule = it.next();
            if (tmp_rule.get_path_type().equals(PathType.Xpath)){
                //进行路径为Xpath格式的处理
                System.out.print("进行Xpath路径处理\n");
                String res = Get_xpath_obj(document,tmp_rule);
                this.result.put(tmp_rule.get_name(),res);
            }else if (tmp_rule.get_path_type().equals(PathType.Htmlpath)){
                //进行路径为原始格式的处理
                System.out.print("进行Html路径处理\n");
                create_obj(document);
                generate_data(tmp_rule);

            }
        }
    }

    //如果要切片的话，需要对此函数进行更改，并需要进行标志位的添加
    public String Get_xpath_obj(String document ,RuleItem xpath_Rul){
        DocType type = this.judge_doc_type(document);
        this.X_obj = new XpathObject(document,type);
        return  this.X_obj.get_value(xpath_Rul.get_path());
    }
	/* 抽取之前必须执行的方法 */
...
//                    System.out.println(ss.substring(0,ss.length()-1));
    ...
    public void generate_data(RuleItem the_rule) {
            RuleItem tmp_rule = the_rule;
            String name = tmp_rule.get_name();
            if (tmp_rule.get_type().equals(DataType.STR)) {
				/* 单个字段 */
                String result_str = this.obj.get_value(tmp_rule.get_path(),
